...
func TestSelectOptimalUTXOs(t *testing.T) {
	...
	selectedUtxos, totalSelectedAmount, withdrawAmount, changeAmount, estimatedFee, _, err := wallet.SelectOptimalUTXOs(utxos, []string{types.WALLET_TYPE_P2WPKH}, 40000000, 0, 100, 1)
	assert.NoError(t, err)
	assert.NotNil(t, selectedUtxos)
	...
	_, _, _, _, estimatedFee, _, err = wallet.SelectOptimalUTXOs(utxos, []string{types.WALLET_TYPE_P2WPKH}, 150000000, 0, 100, 1)
	assert.Error(t, err)
